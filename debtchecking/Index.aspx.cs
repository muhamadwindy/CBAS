...
{
    public partial class Index : DataPage
    {
        #region static vars
        ...
        private System.Text.StringBuilder htmlbuilder = new System.Text.StringBuilder();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                BranchName.Text = Session["BranchName"].ToString();
                GroupName.Text = Session["GroupName"].ToString();
            ...
            while (connESecurity.hasRow())
            {
                ...
                if (loopcounter == 1)
                    if (!IsPostBack)
                        try
                        {
                            AppModule.Text = connESecurity.GetFieldValue(2);
                        }
                        catch (Exception ex1)
                        {
...
data-widget=""treeview"" role=""menu"" data-accordion=""false"">");
            ...
            for (int i = 0; i < dv.Count; i++)
            {
                ...
                if (menuurl != "")
                {
                    htmlbuilder.Append("<li class=\"nav-item\"><a href=\"#\" onclick=\"changeUrl('" + ResolveUrl("~/") + menuurl + "')\" class=\"nav-link\"><i class=\"" + (menustyle == "" ? "fas fa-minus nav-icon" : menustyle) + "\"></i><p>" + menudesc + "</p></a> \n");
